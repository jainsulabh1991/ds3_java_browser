package com.spectralogic.dsbrowser.gui.util;

public interface ImageURLs {
    final String INTERRUPTEDJOBIMAGE = "/images/interruption.png";
    final String CANCELALLJOBIMAGE = "/images/cross.png";

    //Storage locations images
    final String ONLINEDISK = "/images/onlineDisk.png";
    final String NEARLINEDISK = "/images/arcticBlue.png";
    final String STORAGETAPES = "/images/tape.png";
    final String EJECTEDTAPES = "/images/tapeEjected.png";
    final String BLACKPEARLCACHE = "/images/blackPearlCache.png";
    final String REPLICATION = "/images/bpReplicate.png";

    final String LENSICON = "/images/lens.png";
    final String CROSSICON = "/images/cross.png";
    final String SETTINGSICON = "/images/Settings.gif";

    final String RECOVERIMAGE = "/images/recover.png";
    final String CANCELRECOVER = "/images/cancel.png";

    final String JOBICON = "/images/job.png";
    final String FOLDERICON = "/images/folder.png";
    final String FILEICON = "/images/file.png";
    final String CHILDLOADER = "/images/loading.gif";
    final String DEEPSTORAGEBROWSER = "/images/deep_storage_browser.png";
}
